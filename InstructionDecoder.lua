...
InstDecoder.decode_LdrStr = function(addr)
	...
	local Rd = 7
	if InstDecoder.authInstruction(inst, IMM, 3) then
	...
	elseif InstDecoder.authInstruction(inst, IMM_H, 4) then
		local L = bit.lshift(1, 11)
		...
		output["Rb"] = bit.rshift(bit.band(inst, Rb), 3)
		output["Rd"] = bit.band(inst, Rd)
	elseif InstDecoder.authInstruction(inst, REG, 4) then
		-- TODO: test
		local L = bit.lshift(1, 11)
		...
		output["Rb"] = bit.rshift(bit.band(inst, Rb), 3)
		output["Rd"] = bit.band(inst, Rd)
	elseif InstDecoder.authInstruction(inst, REG_H, 4) then
		-- TODO: test
...
InstDecoder.decode_PushPop = function(addr)
	...
	if InstDecoder.authInstruction(inst, PUSHPOP, 4) then
		local L = bit.lshift(1, 11)
